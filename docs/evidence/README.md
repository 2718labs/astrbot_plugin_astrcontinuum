...
`docs/assets/evidence-r2-v03-evidence.svg` is generated by
`scripts/render_versioned_evidence_figure.py` from
`frozen-r2-outcomes.csv` and `v03-update-gate-a/aggregate.csv`. It keeps the
historical R2 outcome panel and the v0.3 Gate A structural-verification panel
separate: there is no shared outcome axis, performance comparison, or E2E
claim.
