...
本文描述 `v0.1.0` 已启用的压缩通道。AstrBot `Star` 会保存由上下文压力触发的意图，
启动唯一受跟踪的后台 worker，并在终止时取消它。因此自动执行已经属于安装后的预览版。
...
1. 切分带角色标签的有界证据；
2. 只让所选 Provider 返回事件编号与逐字来源片段；
3. 拒绝缺失确认、额外字段、未知编号和非逐字文本；
4. 编译不可变结构化 Capsule 并确定性渲染；
5. 校验身份、覆盖、来源、依赖、精确锚点与闭合 envelope；
6. 可选执行语义损失审计；
7. 构造恰好覆盖冻结 target 的候选 Snapshot；
8. 所有强制校验通过后才能进入 `READY_TO_COMMIT`。
...
安装后的预览版已经包含由 `Star` 管理的 claim loop、冻结归约读取、lease 续租、有界
取消、Provider 选择、脱敏重试、原子发布和基础 `/context_status` 遥测。

Provider 语义审计适配器仍未绑定，但强制机械校验与永久发布校验会始终运行。数据库
静态加密和更完整的运维控制仍属于后续安全/生产工作。
