...
class ScaffoldCapsuleCompiler:
    ...
    async def compile(
        ...
        claims = tuple(
            Claim(
                str(uuid.uuid4()), "fact", event.content, ClaimStatus.ACTIVE, 1.0, (event.event_id,)
            )
            for event in events
        )
        return (
            ContextCapsule(
                ...
                events[-1].sequence,
                claims=claims,
                narrative_summary="Scaffold capsule; replace with real compiler.",
                token_cost=sum(max(1, len(e.content) // 4) for e in events),
                source_coverage=1.0,
            ),
        )
