...
from astrcontinuum.models import (
    Claim,
    ClaimStatus,
    ContextCapsule,
    ContextEvent,
...
def raw_event() -> ContextEvent:
    return ContextEvent(
        "e1", "s", 1, "user_message", "user", "仓库名是 Ayleovelle/SylannEngine", 1.0
    )
...
async def test_audit_rejects_missing_anchor_source() -> None:
    capsule = ContextCapsule(
        ...
        1,
        1,
        exact_anchors=(ExactAnchor("a1", "name", "Ayleovelle/SylannEngine", "missing"),),
    )
